...
- apps/api: Hono on Cloudflare Workers / Drizzle ORM / Turso(libSQL) / better-auth / Upstash Redis / Cloudflare R2 / Workers KV / Polar(決済) / Workers AI(生成 gpt-oss-120b・埋め込み embeddinggemma-300m)
...
- キャッシュ（Redis / グローバル共通キャッシュ）と非同期ジョブ（Queue→Worker）の境界を data-flow.md に沿って守る。AI出力はキャッシュしてリクエスト回数を削減する。

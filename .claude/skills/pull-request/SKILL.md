---
name: pull-request
description: 作業ツリーの変更を merge-ready な Pull Request にし、ローカル品質ゲートとレビューが緑になるまで回す。ローカルゲート実行 → PR 作成/更新（このリポジトリの PR 規約準拠）→ レビュー解消ループ。UI 変更は 0.0.0.0:{10000+番号} で起動して動作検証する。「PR を出す」「ship する」「レビューを通す」「レビュー指摘を直す」時に /pull-request で使う。
---

# Pull request: 作成・検証・レビュー通過

現在の変更を、このリポジトリの品質ゲートを通る Pull Request にし、レビュー指摘を緑になるまでループで直すエンドツーエンド手順。緑になったらブログ価値のある変更を提案する（Phase 6）。

このリポジトリの品質ゲート:

1. **ローカルゲート**（push 前に自分で回す）: `pnpm typecheck` / `pnpm lint` / `pnpm build`、api 差分があれば `pnpm --filter api test`（vitest）。
2. **レビュー**（PR のステータスチェック＆レビュースレッド）:
   - **現状このリポジトリには GitHub Actions の CI も AI レビュアー bot も未設定**（`.github` は gitignore）。よって現状の「緑」= ローカルゲート通過 ＋ human レビューで未解消の指摘が無い状態。
   - CI / AI レビュアー（例 `coderabbitai[bot]` / `cubic-dev-ai[bot]`）が導入されたら Phase 4 と `scripts/review-status.sh` がそのまま機能する。bot は `PR_REVIEW_BOTS`（空白区切り）で上書きできる。
...
CI / AI レビュアーが設定済みの場合、`review-status.sh` が `0` で exit するまで繰り返す。**3反復**で打ち切り（Phase 5 参照）。CI/bot 未設定の現状では、ローカルゲート通過後に human レビューへ回し、指摘に同じループで対応する。

1. **ゲートが確定するまで待ち**、読む:

   ```bash
   gh pr checks --watch --interval 30      # チェックが終わる（or 失敗）までブロック（CI がある場合）
   .claude/skills/pull-request/scripts/review-status.sh
   ```

   - exit `0` → 緑。「完了」へ。
   - exit `1` → ブロック。出力に失敗チェックと未解消レビュースレッド（`[reviewer] path:line: コメント`）が並ぶ。続行。
...
| チェック監視 | `gh pr checks --watch --interval 30`（CI がある場合） |
| ゲート判定 | `.claude/skills/pull-request/scripts/review-status.sh [PR#]` |
| PR レビュースレッド（生） | `gh api repos/{owner}/{repo}/pulls/{n}/comments` |

**ゲート扱いの AI レビュアー bot**: 既定 `coderabbitai[bot]` / `cubic-dev-ai[bot]`（現状このリポジトリは未設定）。bot 集合は `PR_REVIEW_BOTS` 環境変数（空白区切り）で上書きする。

...
> 「言葉集めのハードルを極限まで下げる」中核機能。空欄を Gemini が補完し、ユーザー入力を最小化する。AI が生成する意味・例文の品質は後続のクイズ/カードに直結する。
...
- 単語登録時、**テキスト未入力の欄だけ** AI が補完する（入力済みの欄は触らない）。要件定義の「AI補完ON」挙動を実装。
- 登録は即時返し、補完は**非同期**で進行（一覧に戻ってから "補完中→完了" 表示）。
...
- Gemini 2.5 Flash Lite 連携モジュール（意味・品詞・発音記号・例文・コロケーション・類語・語源）。
